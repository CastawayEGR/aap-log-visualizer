...
ADD https://dl.grafana.com/oss/release/grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz /opt
ADD https://github.com/grafana/loki/releases/download/v${LOKI_VERSION}/loki-linux-${TARGETARCH}.zip /opt
ADD https://github.com/grafana/loki/releases/download/v${LOKI_VERSION}/promtail-linux-${TARGETARCH}.zip /opt

RUN tar xf /opt/grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz -C /opt && rm -f /opt/grafana-${GRAFANA_VERSION}.linux-${TARGETARCH}.tar.gz \
...
RUN dnf install unzip -y \
    && mkdir -p /opt/loki/data && unzip /opt/loki-linux-${TARGETARCH}.zip -d /opt/loki && rm -f /opt/loki-linux-${TARGETARCH}.zip \
    && mkdir /opt/promtail && unzip /opt/promtail-linux-${TARGETARCH}.zip -d /opt/promtail && rm -f /opt/promtail-linux-${TARGETARCH}.zip \
